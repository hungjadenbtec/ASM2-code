package com.mycompany.asm2_qlsv;

/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
import java.io.PrintStream;
import java.util.Collections;
import java.util.List;
import java.util.Scanner;
/**
 *
 * @author welcome
 */
public class StudentMng {
    public static Scanner scanner = new Scanner(System.in);
    private List<Student> studentList;
    private final StudentAD studentDAO;
     
    /**
     * init StudentAD object and
 read list student when init StudentManager object
     */   
    public StudentMng() {
        studentDAO = new StudentAD();
        studentList = studentDAO.read();
    }
 
    /**
     * add student to studentList
     * 
     */
    public void add() {
        String id = inputId() ;
        id = scanner.nextLine();
        System.out.println("Student ID = " + id);
        String name = inputName();
        byte age = inputAge();
        String address = inputAddress();
        float gpa = inputGpa();
        String grd;
        String pga = null;
        Student student = new Student(id, name, age, address, gpa, pga);
        studentList.add(student);
        studentDAO.write(studentList);
    }
 
    /**
     * edit student by id
     * 
     * @param id
     */
    public void edit(String id) {
        boolean isExisted = false;
        int size = studentList.size();
        for (int i = 0; i < size; i++) {
            if (studentList.get(i).getId() == null ? id == null : studentList.get(i).getId().equals(id)) {
                isExisted = true;
                studentList.get(i).setName(inputName());
                studentList.get(i).setAge(inputAge());
                studentList.get(i).setAddress(inputAddress());
                studentList.get(i).setGpa(inputGpa());
                break;
            }
        }
        if (!isExisted) {
            System.out.printf("id = %s not existed.\n", id);
        } else {
            studentDAO.write(studentList);
        }
    }
 
    /**
     * delete student by id
     * 
     * @param id: student id
     */
    public void delete(String id) {
        Student student = null;
        int size = studentList.size();
        for (int i = 0; i < size; i++) {
            if (studentList.get(i).getId() == null ? id == null : studentList.get(i).getId().equals(id)) {
                student = studentList.get(i);
                break;
            }
        }
        if (student != null) {
            studentList.remove(student);
            studentDAO.write(studentList);
        } else {
            PrintStream PrintStream; /*printf*/
            PrintStream = System.out.printf("id = %s not existed.\n", id);
        }
    }
 
    /**
     * sort student by name
     */
    public void sortStudentByName() {
        Collections.sort(studentList, new StudentNO());
    }
 
    /**
     * sort student by id
     */
    public void sortStudentByGPA() {
        Collections.sort(studentList, new StudentAP());
    }
 
    /**
     * show list student to screen
     */
    public void show() {
        for (Student student : studentList) {
            System.out.format("|%5s | ", student.getId());
            System.out.format("|%20s | ", student.getName());
            System.out.format("|%5d | ", student.getAge());
            System.out.format("|%20s | ", student.getAddress());
            System.out.format("|%5s | ", student.getGpa());
            System.out.format("|%10.1s%n", student.getPga());
        }
    }
 
    /**
     * input student id
     * 
     * @return student id
     */
    public String inputId() {
        String id ;
        id = scanner.nextLine();
        while (true) {
            try {
                        System.out.print("Input Student ID: " + id);
                        id = scanner.nextLine();
                return id;
            } catch (NumberFormatException ex) {
                System.out.print("invalid! Input student id again: ");
            }
        }
    }
     
    /**
     * input student name
     * 
     * @return student name
     */
    private String inputName() {
        System.out.print("Input Name: ");
        return scanner.nextLine();
    }
 
    /**
     * input student address
     * 
     * @return student address
     */
    private String inputAddress() {
        System.out.print("Input Address: ");
        return scanner.nextLine();
    }
 
    /**
     * input student age
     * 
     * @return student age
     */
    private byte inputAge() {
        System.out.print("Input Age: ");
        while (true) {
            try {
                byte age = Byte.parseByte((scanner.nextLine()));
                if (age < 0 && age > 100) {
                    throw new NumberFormatException();
                }
                return age;
            } catch (NumberFormatException ex) {
                System.out.print("invalid! Input Student ID again: ");
            }
        }
    }
 
    /**
     * input student gpa
     * 
     * @return gpa
     */
    private float inputGpa() {
        System.out.print("Input Student Point: ");
        while (true) {
            try {
                float gpa = Float.parseFloat((scanner.nextLine()));
                if (gpa < 0.0 && gpa > 10.0) {
                    throw new NumberFormatException();
                }
                return gpa;
            } catch (NumberFormatException ex) {
                System.out.print("invalid! Input Age again: ");
            }
        }
    }
 
    // getter && setter
    public List<Student> getStudentList() {
        return studentList;
    }
 
    public void setStudentList(List<Student> studentList) {
        this.studentList = studentList;
    }
}
